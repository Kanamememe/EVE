/** EVE Chat AI Limit Diagnostics v1.2.0 */
(function (window, document) {
  ...
  function classify(item){
    if(item.networkError)return{type:'network',label:'网络或请求错误',severity:'error'};
  ...
  function init(){if(initialized)return;initialized=true;on(window,'eve:ai-response-detail',event=>add(event.detail||{}));on(window,'eve:ai-error',event=>add({timestamp:Date.now(),url:event.detail?.url||'',networkError:clean(event.detail?.error?.message||event.detail?.error),status:0,ok:false}));window.EVE||={};window.EVE.aiDiagnostics=window.EVEAIDiagnostics;emit('eve:ai-diagnostics-ready',getDiagnostics())}
